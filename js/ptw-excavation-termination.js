// Function to populate Excavation PTW Number select box
async function populatePTWNumberSelect() {
  ...
  try {
    ...
    const querySnapshot = await db.collection('Excavation PTW Issue')
...
async function fetchPTWDetails() {
  ...
  if (selectedPTWId) {
    try {
      const doc = await db.collection('Excavation PTW Issue').doc(selectedPTWId).get();
...
async function saveExcavationPTWTerminationFormData(event) {
  event.preventDefault();

  if (!confirm('Do you want to submit the Excavation PTW Termination?')) {
    return;
  }
  ...
  try {
    ...
    const docRef = db.collection('Excavation PTW Termination').doc(terminationNumber);
    await docRef.set(formData);

    // Update the status of the Excavation PTW Issue to 'Closed'
    await db.collection('Excavation PTW Issue').doc(selectedPTWId).update({ status: 'Closed' });
    console.log('Excavation PTW status updated to Closed');

    // Update the status of all extended permits in Excavation PTW Extension to 'Closed'
    const extensionQuerySnapshot = await db.collection('Excavation PTW Extension')
    ...
    extensionQuerySnapshot.forEach(doc => {
      const docRef = db.collection('Excavation PTW Extension').doc(doc.id);
      batch.update(docRef, { status: 'Closed' });
    });

    await batch.commit();
    console.log('All extended Excavation PTW statuses updated to Closed');

    const captureSection = document.getElementById('capture-section');
    html2canvas(captureSection, { useCORS: true, scale: 2 }).then(async (canvas) => {
      canvas.toBlob(async (blob) => {
        const storageRef = firebase.storage().ref();
        const fileRef = storageRef.child(`excavation_ptw_forms/${terminationNumber}.jpeg`);
        ...
        ptwForm.reset();
        hideLoading();
        alert('Excavation PTW successfully submitted with ID: ' + terminationNumber);
      ...
      console.error('Error capturing Excavation PTW form image: ', error);
      hideLoading();
      alert('Error capturing Excavation PTW form image: ' + error.message);
    });
...
document.getElementById('ptwForm').addEventListener('submit', saveExcavationPTWTerminationFormData);
