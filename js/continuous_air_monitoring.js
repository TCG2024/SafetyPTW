...
async function populatePTWNumberSelect() {
  ...
  console.log(ptwNumberSelect); // Log the selected element
  
  if (!ptwNumberSelect) {
    console.error('Element #ptwNumber not found in the DOM.');
    return;
  }
  ...
  try {
    ...
    console.log(issueSnapshot.docs.length); // Log the number of returned documents
    ...
    for (const doc of issueSnapshot.docs) {
      ...
      if (ptwData.status === 'Open') {
        ...
        console.log(option); // Log the created option
    ...
    initializeSelect2();
    console.log('Select2 Initialized'); // Log after Select2 initialization
  } catch (error) {
...
function addRow() {
  ...
  const monitoringPerson = newRow.insertCell(2);
  ...
  monitoringPerson.innerHTML = `<input type="text" name="monitoringPerson[]" value="${document.getElementById('issuerName').value}" readonly>`;
    ...
    if (!snapshot.empty) {
      snapshot.forEach(doc => {
        ...
        data.readings.forEach((reading, index) => {
          ...
          monitoringPersonCell.innerHTML = `<input type="text" name="monitoringPerson[]" value="${reading.monitoringPerson}" readonly>`;
          dateCell.innerHTML = `<input type="text" name="date[]" value="${reading.date}" readonly>`;
