...
async function fetchGeneralPTWDetails() {
  const ptwNumberSelect = document.getElementById('ptwNumber');
...
async function checkForExtensions(ptwNumber) {
  try {
    ...
    const querySnapshot = await db.collection('General PTW Extension')
...
async function saveGeneralPTWExtensionFormData(event) {
...
const selectedPTWId = document.getElementById('ptwNumber').value; // Define the selectedPTWId here
    ...
    const docRef = db.collection('General PTW Extension').doc(ptwNumber);
    await docRef.set(formData);

// Update the status of the General PTW Issue to 'Extended'
    await db.collection('General PTW Issue').doc(selectedPTWId).update({ status: 'Extended' });
    console.log('PTW status updated to Extended');
...
document.getElementById('ptwForm').addEventListener('submit', saveGeneralPTWExtensionFormData);

//**************************************************************************************************************************************************

// Call functions when document is ready
document.addEventListener('DOMContentLoaded', function() {
  populateGeneralPTWNumberSelect(); // Populate PTW Number select box on page load
  document.getElementById('ptwNumber').addEventListener('change', fetchGeneralPTWDetails); // Fetch PTW details on selection change
});
...
$(document).ready(function() {
  ...
  $('#ptwNumber').on('change', fetchGeneralPTWDetails);
});
