...
async function populateExcavationPTWNumberSelect() {
...
async function fetchExcavationPTWDetails() {
  const ptwNumberSelect = document.getElementById('ptwNumber');
...
async function checkForExtensions(ptwNumber) {
  try {
    ...
    const querySnapshot = await db.collection('Excavation PTW Extension')
...
async function saveExcavationExtensionFormData(event) {
  event.preventDefault();

  if (!confirm('Do you want to generate the Excavation PTW Extension?')) {
    return;
  }
  ...
  try {
    ...
    const docRef = db.collection('Excavation PTW Extension').doc(ptwNumber);
    await docRef.set(formData);

    // Update the status of the Excavation PTW Issue to 'Extended'
    if (selectedPTWId) {
      await db.collection('Excavation PTW Issue').doc(selectedPTWId).update({ status: 'Extended' });
      console.log('Excavation PTW status updated to Extended');
    }
...
document.getElementById('ptwForm').addEventListener('submit', saveExcavationExtensionFormData);
...
document.addEventListener('DOMContentLoaded', function() {
  populateExcavationPTWNumberSelect(); // Populate PTW Number select box on page load
  document.getElementById('ptwNumber').addEventListener('change', fetchExcavationPTWDetails); // Fetch PTW details on selection change
});
...
$(document).ready(function() {
  ...
  $('#ptwNumber').on('change', fetchExcavationPTWDetails);
});
