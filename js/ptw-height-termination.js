// Function to populate Height PTW Number select box
async function populateHeightPTWNumberSelect() {
  const ptwNumberSelect = document.getElementById('ptwNumber');

  try {
    // Fetch PTW records with status 'Open' or 'Extended'
    const querySnapshot = await db.collection('Height PTW Issue')
...
async function fetchPTWDetails() {
  ...
  if (selectedPTWId) {
    try {
      const doc = await db.collection('Height PTW Issue').doc(selectedPTWId).get();
...
async function saveHeightPTWTerminationFormData(event) {
  event.preventDefault();

  if (!confirm('Do you want to submit the Height PTW Termination?')) {
    return;
  }
  ...
  try {
    ...
    const docRef = db.collection('Height PTW Termination').doc(terminationNumber);
    await docRef.set(formData);

    // Update the status of the Height PTW Issue to 'Closed'
    await db.collection('Height PTW Issue').doc(selectedPTWId).update({ status: 'Closed' });
    console.log('Height PTW status updated to Closed');

    // Update the status of all extended permits in Height PTW Extension to 'Closed'
    const extensionQuerySnapshot = await db.collection('Height PTW Extension')
    ...
    extensionQuerySnapshot.forEach(doc => {
      const docRef = db.collection('Height PTW Extension').doc(doc.id);
      batch.update(docRef, { status: 'Closed' });
    });

    await batch.commit();
    console.log('All extended Height PTW statuses updated to Closed');

    const captureSection = document.getElementById('capture-section');
    html2canvas(captureSection, { useCORS: true, scale: 2 }).then(async (canvas) => {
      canvas.toBlob(async (blob) => {
        const storageRef = firebase.storage().ref();
        const fileRef = storageRef.child(`height_ptw_forms/${terminationNumber}.jpeg`);
        ...
        ptwForm.reset();
        hideLoading();
        alert('Height PTW successfully submitted with ID: ' + terminationNumber);
      ...
      console.error('Error capturing Height PTW form image: ', error);
      hideLoading();
      alert('Error capturing Height PTW form image: ' + error.message);
    });

  } catch (error) {
    console.error('Error adding Height PTW form data: ', error);
    hideLoading();
    alert('Error submitting Height PTW: ' + error.message);
  }
}
...
document.getElementById('ptwForm').addEventListener('submit', saveHeightPTWTerminationFormData);
