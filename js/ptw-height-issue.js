...
// Function to auto-generate Height PTW Number
async function generateHeightPTWNumber() {
  try {
    console.log('Generating Height PTW Number');
    ...
    const ptwRef = db.collection('Height PTW Issue');

    // Query to get the PTW documents with the selected general PTW number and "-HT-"
    const snapshot = await ptwRef.where('ptwNumber', '>=', `${selectedGeneralPTWNumber}-HT-`)
                                 .where('ptwNumber', '<', `${selectedGeneralPTWNumber}-HU`)
                                 .orderBy('ptwNumber', 'desc')
                                 .get();
    
    console.log('Query executed, processing results');

    let newPTWNumber = `${selectedGeneralPTWNumber}-HT-1`; // Default if no documents found

    if (!snapshot.empty) {
      let highestNumber = 0;
      snapshot.forEach(doc => {
        const ptwNumberParts = doc.data().ptwNumber.split('-HT-');
      ...
      newPTWNumber = `${selectedGeneralPTWNumber}-HT-${highestNumber + 1}`; // Generate new PTW number
    }
    ...
    console.error('Error fetching PTW numbers: ', error);
    alert('Error fetching Height PTW Numbers: ' + error.message);
  }
}
...
async function saveHeightPTWFormData(event) {
  event.preventDefault();

  if (!confirm('Do you want to generate the Height PTW?')) {
    return;
  }
  ...
  try {
    ...
    const docRef = db.collection('Height PTW Issue').doc(ptwNumber);
    ...
    html2canvas(captureSection, { useCORS: true, scale: 2 }).then(async (canvas) => {
      canvas.toBlob(async (blob) => {
        ...
        const fileRef = storageRef.child(`height_ptw_forms/${ptwNumber}.jpeg`);
        ...
        ptwForm.reset();
        hideLoading();
        alert('Height PTW successfully submitted with ID: ' + ptwNumber);
...
document.getElementById('ptwForm').addEventListener('submit', saveHeightPTWFormData);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  saveHeightPTWFormData(); // Save PTW form data on form submission
