...
async function fetchHeightPTWDetails() {
  const ptwNumberSelect = document.getElementById('ptwNumber');
...
async function checkForExtensions(ptwNumber) {
  try {
    ...
    const querySnapshot = await db.collection('Height PTW Extension')
...
async function saveHeightExtensionFormData(event) {
  event.preventDefault();

  if (!confirm('Do you want to generate the Height PTW Extension?')) {
    return;
  }
  ...
  try {
    ...
    const docRef = db.collection('Height PTW Extension').doc(ptwNumber);
    await docRef.set(formData);

    // Update the status of the Height PTW Issue to 'Extended'
    if (selectedPTWId) {
      await db.collection('Height PTW Issue').doc(selectedPTWId).update({ status: 'Extended' });
      console.log('Height PTW status updated to Extended');
    }
...
document.getElementById('ptwForm').addEventListener('submit', saveHeightExtensionFormData);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('ptwNumber').addEventListener('change', fetchHeightPTWDetails); // Fetch PTW details on selection change
});
...
$(document).ready(function() {
  ...
  $('#ptwNumber').on('change', fetchHeightPTWDetails);
});
